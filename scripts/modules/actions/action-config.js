...
// slice 5: ACTION_EFFECTS for Universal Table columns derived from the kernel
// EFFECT_COLUMNS via ACTION_COLUMNS; sub-table and save rows stay literal.

import { EFFECT_COLUMNS } from "../../lib/faserip-rules/faserip-effects.js";
import { labelForToken } from "../../kernel/adapter.js";
...
const columnEffects = (code) => Object.fromEntries(
  Object.entries(EFFECT_COLUMNS[code].results).map(([color, token]) => [color, labelForToken(token)])
);

export const ACTION_EFFECTS = {
  ...Object.fromEntries(Object.entries(ACTION_COLUMNS).map(([type, code]) => [type, columnEffects(code)])),
