// scripts/modules/actions/force-action.js v3.0.0 - 2026-03-16
...
// v1.4.0: Fix CS persistence - decouple from global rememberSettings
// v1.3.7: Add effect modifier system - read target status (Stunned +2CS, etc)
// v1.3.6: Add CS breakdown tooltip showing Manual/Range/Obstacle/Movement/Adjacent
...
export class ForceAction extends RangedAttackAction {
  async execute() {
    ...
    // === Detect force-relevant combat talents ===
    const combatTalents = [];
    ...
    const savedActiveChips = shouldRemember ? (await actor.getFlag("msh-faserip", "lastForceActiveChips") || {}) : {};
    ...
    <div class="frp-dlg">
      ...
      <div class="frp-box frp-cs-box">
        <div class="frp-cs-line">
          ...
          <input type="number" class="frp-cs-input${csInputCls}" name="shift" value="${savedColumnShift}" id="cs-force">
          <span class="frp-cs-arrow">&rarr;</span>
          <span class="frp-cs-rank" id="rank-force" style="${csRankStyle}">${shiftRank(initialDisplayRank, savedColumnShift)}</span>
      ...
      <div class="frp-box" style="padding:3px 8px;background:#e3f2fd;border-color:#90caf9;">
        ...
        <div style="margin-top:3px;display:flex;align-items:center;gap:6px;font-size:12px;">
          ...
          <select name="targetMovement" style="font-size:12px;padding:2px 3px;border:1px solid #b8b8b8;border-radius:2px;">
            ...
            <option value="-2">Running (&minus;2CS)</option>
            <option value="-3">Dodging (&minus;3CS)</option>
          </select>
        </div>
      </div>
      ...
      new Dialog({
          ...
          roll: {
            ...
            callback: async (html) => {
              ...
              if (rememberSettings) {
                ...
                await actor.setFlag("msh-faserip", "lastForceActiveChips", activeChips);
              }
        ...
        render: async (html) => {
          ...
          const update = () => {
            ...
            const $shiftedRank = html.find('#rank-force');
