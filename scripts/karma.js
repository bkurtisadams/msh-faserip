...
export class KarmaSheet extends DocumentSheet {
    ...
    _onClearKarma(event) {
        ...
        new Dialog({
          ...
          content: `
            <p>Are you sure you want to delete ALL karma history entries? This cannot be undone.</p>
            <div class="form-group">
              <label>What should happen to Current and Lifetime Karma totals?</label>
              <select name="resetTotals">
                <option value="keep">Keep current values</option>
          ...
          buttons: {
            delete: {
              ...
              label: "Delete All",
              callback: (html) => {
                const resetOption = html.find('[name="resetTotals"]').val();
                ...
                // If user chose to reset totals, set them to zero
                if (resetOption === "reset") {
                  updateData["system.attributes.karma.value"] = 0;
                  updateData["system.karma.lifetime"] = 0;
                }
                
                // Update the actor
                this.object.update(updateData).then(() => {
                  ui.notifications.info("Karma history has been cleared.");
                  this.render(true);
                });
              }
            },
    ...
    _onImportKarma(event) {
        ...
        input.type = 'file';
        input.accept = '.csv,.tsv,.txt'; // Accept CSV, TSV, and general text files
        ...
        input.onchange = e => {
          ...
          if (file.size === 0) {
            ui.notifications.error("The selected file is empty.");
            return;
          }
    ...
    async _importCSV(file) {
        ...
        reader.onload = async (e) => {
          ...
          // Remove BOM if present and handle line endings
          const content = text.replace(/^\uFEFF/, '');
          const lines = content.split(/\r?\n/);
          
          console.log("Number of lines:", lines.length);
          if (lines.length === 0 || !lines[0].trim()) {
            ui.notifications.error("The file appears to be empty.");
            return;
          }
          ...
          if (lines[0].includes('\t')) {
            ...
            delimiter = ';';
            console.log("Using semicolon as delimiter");
          }
          ...
          if (headers.length < 2) {
            ui.notifications.error("Invalid file format: Could not detect proper columns.");
            return;
          }
          ...
          if (data.length === 0) {
            ui.notifications.warn("No valid data rows found in the file.");
            return;
          }
          
          // Show import dialog with the data
