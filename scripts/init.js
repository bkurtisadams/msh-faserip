...
/**
 * Fallback Timer Manager (when CTT is disabled)
 * Uses Foundry combat rounds for timer tracking
 */
class FallbackTimerManager {
  constructor() {
    this.scope = globalThis.MSH_FLAG_SCOPE || "msh-faserip";
  }

  /**
   * Start Recovery timer (10 turns = 10 combat rounds)
   */
  async startRecoveryTimer(actor, currentHealth, maxHealth, enduranceValue) {
    console.log("FASERIP Fallback | startRecoveryTimer called", {
      currentHealth,
      maxHealth,
      enduranceValue
    });

    // Only if conscious and not at max health
    if (currentHealth === 0 || currentHealth >= maxHealth) {
      console.log("FASERIP Fallback | Skipping Recovery - unconscious or at max health");
      return;
    }
    
    const lastRecoveryDate = actor.getFlag(this.scope, "lastRecoveryDate");
    const today = new Date().toDateString();
    
    // Don't start if already used today
    if (lastRecoveryDate === today) {
      console.log("FASERIP Fallback | Already used Recovery today");
      return;
    }
    
    // Remove existing timer
    const existingRecovery = actor.effects.find(e => e.flags?.[this.scope]?.recoveryTimer);
    if (existingRecovery) {
      await actor.deleteEmbeddedDocuments("ActiveEffect", [existingRecovery.id]);
    }
    
    // Create effect with combat rounds
    const effectData = {
      name: "Recovery Timer (10 turns)",
      img: "icons/svg/clockwork.svg",
      origin: actor.uuid,
      flags: {
        [this.scope]: {
          recoveryTimer: true,
          healAmount: enduranceValue,
    ...
    console.log(`FASERIP Fallback | Recovery timer started for ${actor.name} (10 turns, +${enduranceValue} HP)`);
  }

  /**
   * Start Healing timer (600 turns = 600 combat rounds)
   */
  async startHealingTimer(actor, currentHealth, maxHealth, enduranceValue) {
    console.log("FASERIP Fallback | startHealingTimer called", {
      actorName: actor.name,
      currentHealth,
      maxHealth
    });

    // Skip healing while at 0 HP
    if (currentHealth <= 0) {
      console.log("FASERIP Fallback | At 0 HP — skipping Healing timer");
      return;
    }
    
    if (currentHealth >= maxHealth) {
      console.log("FASERIP Fallback | At max health, stopping healing cycle");
      return;
    }
    
    // Check for existing healing timer
    const existingHealing = actor.effects.find(e => e.flags?.[this.scope]?.healingTimer);
    if (existingHealing) {
      console.log("FASERIP Fallback | Healing timer already exists");
      return;
    }
    
    const hasMedicalCare = actor.getFlag(this.scope, "medicalCare") || false;
    const healAmount = hasMedicalCare ? enduranceValue * 2 : enduranceValue;
    
    const effectData = {
      name: hasMedicalCare ? "Healing (Medical Care) (600 turns)" : "Healing (600 turns)",
    ...
    await actor.createEmbeddedDocuments("ActiveEffect", [effectData]);
    console.log("FASERIP Fallback | Healing timer created");
    
    // Post chat card
    const currentCare = hasMedicalCare ? "ON" : "OFF";
    const careStatus = hasMedicalCare ? "2x Healing Rate" : "Normal Healing Rate";

    ChatMessage.create({
      content: `
        <div style="background:#f5f5f0;border:1px solid #c0c0c0;border-radius:3px;margin-bottom:5px;">
          <div style="padding:5px 10px;border-bottom:1px solid #c0c0c0;font-size:1.1em;color:#1565c0;">
            <strong>${actor.name} — Healing Timer Started</strong>
          </div>
          
          <div style="padding:5px 10px;font-size:.9em;">
            <div style="margin-bottom:4px;">
              <strong>Healing Amount:</strong> ${healAmount} HP per hour
            </div>
            <div style="margin-bottom:4px;">
              <strong>Medical Care:</strong> ${currentCare} (${careStatus})
            </div>
            <div style="color:#666;">
              Next healing in 600 turns (1 hour)
            </div>
          </div>

          <div style="padding:8px 10px;margin:8px 10px;background:#e3f2fd;border:1px solid #2196f3;border-radius:3px;">
            <div style="font-weight:bold;color:#1565c0;margin-bottom:4px;">
              <i class="fas fa-medkit"></i> Medical Care Toggle
            </div>
            <div style="font-size:.85em;color:#1976d2;margin-bottom:8px;">
              Medical care doubles healing rate (${enduranceValue} HP → ${enduranceValue * 2} HP per hour)
            </div>
            <button class="toggle-medical-care" 
                    data-actor-id="${actor.id}"
                    style="width:100%;background:#2196f3;color:white;border:none;padding:8px;border-radius:4px;cursor:pointer;font-weight:bold;font-size:1em;">
              <i class="fas fa-hospital"></i> Toggle Medical Care (Currently: ${currentCare})
            </button>
          </div>
        </div>
      `,
   ...
   * Complete Recovery
   */
  async completeRecovery(actor, flags) {
    ...
    if (currentHealth === 0) {
      ChatMessage.create({
        content: `
          <div style="background:#f5f5f0;border:1px solid #c0c0c0;border-radius:3px;margin-bottom:5px;">
            <div style="padding:5px 10px;border-bottom:1px solid #c0c0c0;font-size:1.1em;color:#c62828;">
              <strong>${actor.name} — Recovery Failed</strong>
            </div>
            
            <div style="padding:6px 10px;margin:6px 10px;background:#ffebee;border:1px solid #ef5350;border-radius:3px;">
              <div style="font-weight:bold;color:#c62828;">Unconscious</div>
              <div style="font-size:.9em;color:#555;">
                Recovery timer cancelled because ${actor.name} became unconscious before completion.
              </div>
            </div>
          </div>
        `,
    ...
    // Apply healing
    const healAmount = flags.healAmount || 0;
    ...
    // Mark as used today
    const today = new Date().toDateString();
    await actor.setFlag(this.scope, "lastRecoveryDate", today);
    
    ChatMessage.create({
      content: `
        <div style="background:#f5f5f0;border:1px solid #c0c0c0;border-radius:3px;margin-bottom:5px;">
          <div style="padding:5px 10px;border-bottom:1px solid #c0c0c0;font-size:1.1em;color:#2e7d32;">
            <strong>${actor.name} — Recovery Complete</strong>
          </div>
          
          <div style="padding:5px 10px;font-size:.9em;">
            <div style="margin-bottom:4px;">
              <strong>Health Recovered:</strong> ${healAmount} HP
            </div>
            <div style="margin-bottom:4px;">
              <strong>Health:</strong> ${currentHealth} → ${newHealth}
            </div>
            <div style="color:#666;font-size:.85em;">
              Recovery used for today (once per day limit)
            </div>
          </div>

          <div style="text-align:center;padding:8px;margin:5px;font-weight:bold;font-size:1.05em;border-radius:3px;background:#4caf50;color:white;">
            <i class="fas fa-heart"></i> RECOVERED ${healAmount} HEALTH
          </div>
        </div>
      `,
      speaker: ChatMessage.getSpeaker({ actor })
    ...
    ui.notifications.info(`${actor.name} recovered ${healAmount} Health!`);
  }

  /**
   * Complete Healing
   */
  async completeHealing(actor, flags) {
    const currentHealth = actor.system.attributes?.health?.value ?? 0;
    const healAmount = flags.healAmount || 0;
    const maxHealth = actor.system.attributes?.health?.max ?? 0;
    const newHealth = Math.min(currentHealth + healAmount, maxHealth);
    const medicalCare = flags.medicalCare || false;
    const baseEndurance = flags.baseEndurance || (healAmount / (medicalCare ? 2 : 1));
    
    await actor.update({"system.attributes.health.value": newHealth});
    
    const careText = medicalCare ? " (Medical Care)" : "";
    const careIcon = medicalCare ? '<i class="fas fa-hospital"></i>' : '<i class="fas fa-medkit"></i>';

    ChatMessage.create({
      content: `
        <div style="background:#f5f5f0;border:1px solid #c0c0c0;border-radius:3px;margin-bottom:5px;">
          <div style="padding:5px 10px;border-bottom:1px solid #c0c0c0;font-size:1.1em;color:#1565c0;">
            <strong>${actor.name} — Healing Complete${careText}</strong>
          </div>
          
          <div style="padding:5px 10px;font-size:.9em;">
            <div style="margin-bottom:4px;">
              <strong>Health Healed:</strong> ${healAmount} HP
            </div>
            <div style="margin-bottom:4px;">
              <strong>Health:</strong> ${currentHealth} → ${newHealth}
            </div>
            ${medicalCare ? `<div style="color:#2196f3;font-size:.85em;">Medical care: 2x healing rate</div>` : ''}
          </div>

          <div style="text-align:center;padding:8px;margin:5px;font-weight:bold;font-size:1.05em;border-radius:3px;background:#2196f3;color:white;">
            ${careIcon} HEALED ${healAmount} HEALTH
          </div>
        </div>
      `,
      speaker: ChatMessage.getSpeaker({ actor })
    ...
    ui.notifications.info(`${actor.name} healed ${healAmount} Health!${careText}`);
    
    // If still injured, start another healing timer
    if (newHealth < maxHealth) {
      console.log("FASERIP Fallback | Still injured, continuing healing cycle");
      await this.startHealingTimer(actor, newHealth, maxHealth, baseEndurance);
    } else {
      console.log("FASERIP Fallback | Fully healed");
      ChatMessage.create({
        content: `<p style="color:#2196f3;"><strong>${actor.name}</strong> is now fully healed!</p>`
      });
    }
  }
}
